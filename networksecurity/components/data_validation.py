...
class DataValidation:
    ...
    def validate_columns(self,dataframe:pd.DataFrame)->bool:
        try:
            ...
            logging.info(f"Data from columns:{len(dataframe.columns)}")
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            test_dataframe = test_dataframe.reindex(columns=train_dataframe.columns, fill_value=0)



            status=self.validate_columns(dataframe=train_dataframe)
            if not status:
                error_message="Train data does not contains all"
            status=self.validate_columns(dataframe=test_dataframe)
            if not status:
                error_message="Test data does not contains all"

            self.dataset_drift(base_df=train_dataframe,current_df=test_dataframe)
